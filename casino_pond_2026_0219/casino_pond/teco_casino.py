...
def replace_placeholders_in_hier(ws_hier_file_path, env_vars):
    ...
    try:
        ...
        placeholders = {
            '$prj': env_vars.get('casino_prj_name', ''),
...
if (-e ./common/flow/flow_casino.yaml) then
    yes | {pond_var}/fm_casino.py -force -monitoring -y -only $1 -flow ./common/flow/flow_casino.yaml
else
    yes | {pond_var}/fm_casino.py -force -monitoring -y -only $1 -flow ../../common/flow/flow_casino.yaml
endif
"""
...
## scott : to make itertion number sync between iteration number and actual iteration_purpose : iteration --> iteration-1
        ...
        for role in role_order:
            ...
            role_env_vars['ws'] = f"{role}___{role_env_vars.get('casino_design_ver', '')}_" \
                                  f"{role_env_vars.get('casino_dk_ver', '')}_" \
                                  f"{role_env_vars.get('casino_tag', '')}"
            ...
            ws_path = os.path.join(
                role_env_vars.get('casino_prj_name', ''),
                f"works_{user_id}",
            ...
            partner_role = 'pd' if role == 'pi' else 'pi'
            partner_ws = f"{partner_role}___{role_env_vars.get('casino_design_ver', '')}_" \
                         f"{role_env_vars.get('casino_dk_ver', '')}_" \
                         f"{role_env_vars.get('casino_tag', '')}"
