...
if ( "${pre_post}" == "post" ) then
    ...
    if ( "${sta_pt_is_flatten}" == "0" ) then
        ...
        setenv BLK0_PD_DONE_FILE ${OUTFEED_PATH}/sdc_core0_wrap/pd___${db_ver}/${sta_pt_sub_block_version_sdc_core0_wrap}/SMC.done
        setenv BLK1_PD_DONE_FILE ${OUTFEED_PATH}/sdc_core1_wrap/pd___${db_ver}/${sta_pt_sub_block_version_sdc_core1_wrap}/SMC.done

        while ( ! -e ${TOP_PD_DONE_FILE} || ! -e ${BLK0_PD_DONE_FILE} || ! -e ${BLK1_PD_DONE_FILE} )
            if ( ! -e ${TOP_PD_DONE_FILE}  ) echo "`date`: waiting export (TOP  : ${TOP_PD_DONE_FILE})"
            if ( ! -e ${BLK0_PD_DONE_FILE} ) echo "`date`: waiting export (BLK0 : ${BLK0_PD_DONE_FILE})"
            if ( ! -e ${BLK1_PD_DONE_FILE} ) echo "`date`: waiting export (BLK1 : ${BLK1_PD_DONE_FILE})"
            sleep 60
        end
    endif
