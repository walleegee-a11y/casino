...
if { [file exist ${COMMON_STA_PT}/set_user_variables.tcl] } {
	redirect -tee ./logs/user_variables.tcl {
		source -echo -verbose ${COMMON_STA_PT}/set_user_variables.tcl
	}
}
...
# lib setup
###############################################
source -echo -verbose ${COMMON_STA_PT}/lib_setup.tcl
...
if { ${pre_post} == "pre" } {
	...
	set SDC [glob -nocomplain -type f ${PRJ_HOME}/outfeeds/${top_design}/pi___*/${pi_ver}/sdc/${top_design}.${mode}.sdc]
}
...
if { ${pre_post} == "pre" } {
    ...
    set max_fanout 32
    source -echo -verbose ${COMMON_STA_PT}/HFN.tcl > ./HFN.log
...
if { ${pre_post} == "pre" } {
    ...
    if { [file exist ${COMMON_STA_PT}/tech/aocv_setup.22eHVG_7T.tcl]} {
         source -echo -verbose ${COMMON_STA_PT}/tech/aocv_setup.22eHVG_7T.tcl
...
source -echo -verbose ${COMMON_STA_PT}/mttv_check.22eHVG_7T.tcl

###############################################
#- Project Check
###############################################
#- data skew report
source -echo -verbose ${COMMON_STA_PT}/prj/data_skew_rpt.tcl
