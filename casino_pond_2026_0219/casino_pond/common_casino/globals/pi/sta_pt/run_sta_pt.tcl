...
if { ${pre_post} == "pre" } {
    ...
    # TODO: netlist name?
        ...
        # --- Dynamic Netlist Search for PRE-NET case ---
        # Default path
        set NETLIST "${DB_PATH}/vers/${db_ver}/design/v/${top_design}.v"
        
        # If not found, search across all versions
        if { ![file exists $NETLIST] } {
            set net_search_pat "${DB_PATH}/vers/*/design/v/${top_design}.v"
            set found_nets [glob -nocomplain $net_search_pat]
            if { [llength $found_nets] > 0 } {
                set NETLIST [lindex $found_nets 0]
                puts "INFO: Top Netlist updated dynamically to: $NETLIST"
            }
        }
        # -----------------------------------------------
        ...
        if { $ovars(sta_pt,is_flatten) == 1 } {
            ...
            foreach sub_block $sub_block_list {
                # --- Dynamic Sub-block Netlist Search ---
                set sub_netlist "${DB_PATH}/vers/${db_ver}/design/v/${sub_block}.v"
                
                if { ![file exists $sub_netlist] } {
                    set sub_search_pat "${DB_PATH}/vers/*/design/v/${sub_block}.v"
                    set found_sub_nets [glob -nocomplain $sub_search_pat]
                    if { [llength $found_sub_nets] > 0 } {
                        set sub_netlist [lindex $found_sub_nets 0]
                        puts "INFO: Sub-block ($sub_block) Netlist found at: $sub_netlist"
                    }
                }
                lappend NETLIST $sub_netlist
                # ----------------------------------------
            }
        }
    } else {
    ...
    # Top netlist
    # Note: PD_OUT_PATH should have been dynamically resolved in previous steps.
    set NETLIST ${PD_OUT_PATH}/netlist/${top_design}.v
    
    # Optional: Double check if Top netlist exists, if not try to find it (Safety net)
    if { ![file exists $NETLIST] } {
         set top_post_pat "${OUTFEED_PATH}/${top_design}/pd___*/${run_ver}/netlist/${top_design}.v"
         set found_top [glob -nocomplain $top_post_pat]
         if { [llength $found_top] > 0 } {
             set NETLIST [lindex $found_top 0]
         }
    }
    ...
    if { $need_subblocks } {
        ...
        foreach sub_block $sub_block_list {
            ...
            # --- Dynamic Sub-block Netlist Search (POST) ---
            # Default path using current db_ver
            set sub_netlist "${OUTFEED_PATH}/${sub_block}/pd___${db_ver}/${sb_ver}/netlist/${sub_block}.v"

            # If not found, use wildcard for db_ver
            if { ![file exists $sub_netlist] } {
                set sub_post_pat "${OUTFEED_PATH}/${sub_block}/pd___*/${sb_ver}/netlist/${sub_block}.v"
                set found_sub_post [glob -nocomplain $sub_post_pat]
                
                if { [llength $found_sub_post] > 0 } {
                    set sub_netlist [lindex $found_sub_post 0]
                    puts "INFO: Post Sub-block ($sub_block) Netlist updated to: $sub_netlist"
                } else {
                     puts "WARNING: Post Sub-block ($sub_block) Netlist NOT found!"
                }
            }
            ...
            while { [gets $fin line] >= 0 } {
                ...
                foreach sub_block $sub_block_list {
                    if { $sub_block == $sub_module } {
                        ...
                        # --- Dynamic SPEF Path Resolution Start ---
                        
                        # Define variables for readability
                        set sb_ver $ovars(sta_pt,sub_block_version,${sub_block})
                        set spef_filename "${sub_block}.spef.${spef_cond}.gz"

                        # 1. First, set the default path using the current db_ver
                        set sub_SPEF "${OUTFEED_PATH}/${sub_block}/pd___${db_ver}/${sb_ver}/spef/${spef_filename}"

                        # 2. If the file does not exist at the default path, search dynamically
                        if { ![file exists $sub_SPEF] } {
                            # Construct a search pattern replacing ${db_ver} with a wildcard (*)
                            # Pattern: .../pd___*/sub_block_ver/spef/filename
                            set spef_search_pat "${OUTFEED_PATH}/${sub_block}/pd___*/${sb_ver}/spef/${spef_filename}"
                            
                            # Execute the search
                            set found_spefs [glob -nocomplain $spef_search_pat]

                            if { [llength $found_spefs] > 0 } {
                                # If found, update sub_SPEF to the first matching path
                                set sub_SPEF [lindex $found_spefs 0]
                                puts "INFO: SPEF not found in current db_ver. Updated to: $sub_SPEF"
                            } else {
                                # Optional: Log a warning if even the dynamic search fails
                                puts "WARNING: SPEF file not found in current db_ver or via dynamic search."
                            }
                        }
                        # --- Dynamic SPEF Path Resolution End ---
...
if { [regexp misn ${mode}] } {
	source -echo -verbose ${RUN_COMMON_STA_PT}/prj/data_skew_rpt.tcl
}
