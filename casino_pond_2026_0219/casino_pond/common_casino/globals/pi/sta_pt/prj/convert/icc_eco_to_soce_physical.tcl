proc Conv_ICC_to_SOCE_physical { ICC_ECO SOCE_ECO } {
    ...
    while { [gets $infile line] != -1 } {
        ...
        if { [llength $nline] == 2 && [lindex $nline 0] == "current_instance" } {
            set c_instance [string trim [lindex $nline 1]]
            continue
        }
...
## for sub instance 
        if { $c_instance != ""} {
            ...
            if { ${icc2_cmd} == "insert_buffer" } {
                ...
                set tpin [string trim [lindex $nline $n_pin]]
                set buf  [string trim [lindex $nline [expr $n_pin + 1]]]
                set pin "$c_instance/$tpin"
                set tmp_cell [string trim [lindex $nline [expr [lsearch -exact $nline "-new_cell_names"] + 1]]]
...
# scott
     echo "ecoAddRepeater -term $pin -cell $buf -hinstGuide $c_instance -name $new_cell -loc \{ [lindex [lindex $nline $n_location] 0] [lindex [lindex $nline $n_location] 1] \} -bufOrient $c_orient " >> $SOCE_ECO

            } elseif { ${icc2_cmd} == "size_cell" } {
                set prev_cmd ${icc2_cmd}
                set tcell   [string trim [lindex $nline 1]]
                set newref  [string trim [lindex $nline 2]]
                ...
                set tcell   [string trim [lindex $nline $n_cell]]
                set cell $c_instance/$tcell
                set num_set_cl 0
...
#                   set t_locs [concat $t_locs "[lindex $t_cells [expr $t*5+3]]\}"]
                    set t_names [string trim [lindex $t_cells [expr $t*5]]]
                    set t_refs  [string trim [lindex $t_cells [expr $t*5+1]]]
                    set t_locs "[lindex $t_cells [expr $t*5+2]]"
                    set t_locs [concat $t_locs "[lindex $t_cells [expr $t*5+3]]"]
                    echo "ecoAddRepeater -net \[get_property \[get_net -of $c_instance/$start_pin\] full_name\] -hinstGuide $c_instance -name \{$t_names\} -cell \{$t_refs\} -offLoadAtLoc \{$t_locs\}" >> $SOCE_ECO
                    set t_names ""
                    set t_refs ""
                    set t_locs ""
                }       

#               echo "ecoAddRepeater -net \[get_attribute \[get_net -of $c_instance/$start_pin\] full_name\] -name \{$t_names\} -cell \{$t_refs\} -loc \{$t_locs\}" >> $SOCE_ECO
...
#puts "DBG 4 : $start_pin"
                ...
                for { set t 0} { $t < [expr [llength $t_cells]/5] } {incr t} {
                    ...
                    set t_locs "[lindex $t_cells [expr $t*5+2]]"
                    set t_locs [concat $t_locs "[lindex $t_cells [expr $t*5+3]]"]
                    echo "ecoAddRepeater -net \[get_property \[get_net -of $start_pin\] full_name\] -name \{$t_names\} -cell \{$t_refs\} -offLoadAtLoc \{$t_locs\}" >> $SOCE_ECO
                ...
                #echo "ecoAddRepeater -net \[get_attribute \[get_net -of $start_pin\] full_name\] -name \{$t_names\} -cell \{$t_refs\} -loc \{$t_locs\}" >> $SOCE_ECO
