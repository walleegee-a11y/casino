...
# 1. Set the default path (based on current db_ver)
...
if { ![file isdirectory $PD_OUT_PATH] } {
    
    # 3. If not found, search for other db_ver folders using a wildcard
    #    Pattern: .../pd___*/run_ver
    set search_pattern "${PRJ_HOME}/outfeeds/${top_design}/pd___*/${run_ver}"
    
    # glob -nocomplain: Do not throw an error if no match is found
    # -type d: Look for directories only
    set found_paths [glob -nocomplain -type d $search_pattern]
    
    # 4. If a path is found, update PD_OUT_PATH with the first match
    if { [llength $found_paths] > 0 } {
        set PD_OUT_PATH [lindex $found_paths 0]
        puts "Info: PD_OUT_PATH updated to $PD_OUT_PATH"

		# 5. Define the done file path based on the resolved PD_OUT_PATH
		set PD_DONE_FILE      "${PD_OUT_PATH}/SMC.done"
    }
}
