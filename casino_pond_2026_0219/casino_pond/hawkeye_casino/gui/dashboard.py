...
if GUI_AVAILABLE:
    ...
    class HawkeyeDashboard(QMainWindow):
            ...
            if hasattr(self.analyzer, 'config') and 'tasks' in self.analyzer.config:
                for task_name, task_config in self.analyzer.config['tasks'].items():
                    ...
                    for keyword_config in task_config.get('keywords', []):
                        ...
                        # CRITICAL: Skip template placeholders (unexpanded keywords with {mode}, {corner}, {path_type})
                        # These will be expanded at runtime or are base keywords without path_type suffix
                        if '{' in keyword_name:
                            # Skip unexpanded templates (e.g., "{mode}_{corner}_h_wns_{path_type}")
                            continue

                        # Skip keywords with extract_all_path_types flag (runtime expansion)
                        if keyword_config.get('extract_all_path_types', False):
                            # These are base keywords like "misn_ss_0p72v_m40c_Cmax_h_wns" (no path_type suffix)
                            # The actual keywords with path_types will be collected from run data
                            continue
        ...
        def collect_keywords_from_visible_rows(self) -> List[str]:
            """Collect keywords that appear in currently visible rows (after job/task filtering)

            This ensures keyword groups only show keywords relevant to the current view.
            For example, when viewing apr_inn jobs, only APR keyword groups are shown.
            """
            keywords = set()
            ...
            # Iterate through runs_data and collect keywords from matching jobs/tasks
            for run_path, run_data in self.runs_data.items():
                if 'jobs' in run_data:
                    for job_name, job_data in run_data['jobs'].items():
                        # Check if job matches filter
                        if job_filter != ['all'] and not self.matches_filter(job_name, job_filter):
                            continue

                        for task_name, task_data in job_data.get('tasks', {}).items():
                            # Check if task matches filter
                            if task_filter != ['all'] and not self.matches_filter(task_name, task_filter):
                                continue

                            # Collect keywords from this task
                            for keyword_name in task_data.get('keywords', {}).keys():
                                keywords.add(keyword_name)
                elif 'tasks' in run_data:
                    for task_name, task_data in run_data.get('tasks', {}).items():
        ...
        def load_yaml_config_and_group_keywords(self):
            ...
            try:
                ...
                if self.yaml_config:
                    ...
                    # CRITICAL: Filter keywords to only include those from currently visible rows
                    # This ensures that when viewing apr_inn, we only see apr_inn keyword groups
                    # (not sta_pt groups), and vice versa
                    visible_keywords = self.collect_keywords_from_visible_rows()
                    if visible_keywords:
                        keywords_to_group = visible_keywords
                        debug_print(f"Grouping {len(keywords_to_group)} keywords from visible rows", "UI")
                    else:
                        # Fallback to all keywords if no visible rows
                        keywords_to_group = self.all_keywords
                        debug_print(f"No visible rows, grouping all {len(keywords_to_group)} keywords", "UI")

                    # Group keywords using YAML configuration
                    self.keyword_groups = group_keywords_by_yaml(keywords_to_group, self.yaml_config)
...
#               self.table.setColumnWidth(i, current_width + 20)
        ...
        def apply_filters(self):
            ...
            # CRITICAL: Regenerate keyword groups when filters change
            # This ensures only keywords from visible rows are grouped
            # (e.g., when viewing apr_inn, only APR groups are shown, not STA groups)
...
#                                   return f"{value:.4e}"
#                               # Regular numbers - show up to 4 decimal places, strip trailing zeros
...
"""
        ...
        def show_chart_dialog(self, table):
            ...
            headers = self.generate_dynamic_headers()
            x_axis_options = [
                (Columns.RUN_VERSION, "Run Ver"),
                (Columns.JOB, "Job"),
                (Columns.TASK, "Task"),
            ]
            for col_idx, label in x_axis_options:
                self.x_axis_combo.addItem(label, col_idx)
            # Restore previous selection by actual column index
            combo_idx = self.x_axis_combo.findData(self.chart_settings['x_axis_index'])
            self.x_axis_combo.setCurrentIndex(combo_idx if combo_idx >= 0 else 0)
            ...
            visible_col_count = sum(
                1 for i, _ in enumerate(headers[Columns.PATH_COLUMN_COUNT:], Columns.PATH_COLUMN_COUNT)
                if not self.table.isColumnHidden(i)
            )
            total_col_count = len(headers) - Columns.PATH_COLUMN_COUNT
            main_filter_info = QLabel(f"Showing {visible_col_count} of {total_col_count} columns from main view")
            ...
            # Store visible keyword column info for filtering (mirrors main window's active filters)
            ...
            self.show_data_labels_cb = QCheckBox("Show Numbers")
            self.show_data_labels_cb.setChecked(self.chart_settings['show_data_labels'])
            self.show_data_labels_cb.setToolTip("Show numeric values on chart data points")
            options_layout.addWidget(self.show_data_labels_cb)

            self.log_scale_cb = QCheckBox("Log Scale Y")
            self.log_scale_cb.setChecked(self.chart_settings['log_scale'])
            self.log_scale_cb.setToolTip("Use logarithmic scale for Y axis")
            options_layout.addWidget(self.log_scale_cb)

            custom_layout.addLayout(options_layout)

            # Sort / Y Range / Reference Line row
            options2_layout = QHBoxLayout()

            options2_layout.addWidget(QLabel("Sort:"))
            self.sort_combo = QComboBox()
            self.sort_combo.addItems(["None", "Ascending", "Descending"])
            self.sort_combo.setCurrentText(self.chart_settings['sort_order'])
            self.sort_combo.setToolTip("Sort X-axis bars by first selected Y column")
            options2_layout.addWidget(self.sort_combo)

            options2_layout.addSpacing(12)
            options2_layout.addWidget(QLabel("Y Min:"))
            self.y_min_input = QLineEdit(self.chart_settings['y_min'])
            self.y_min_input.setPlaceholderText("auto")
            self.y_min_input.setFixedWidth(70)
            options2_layout.addWidget(self.y_min_input)

            options2_layout.addWidget(QLabel("Y Max:"))
            self.y_max_input = QLineEdit(self.chart_settings['y_max'])
            self.y_max_input.setPlaceholderText("auto")
            self.y_max_input.setFixedWidth(70)
            options2_layout.addWidget(self.y_max_input)

            options2_layout.addSpacing(12)
            options2_layout.addWidget(QLabel("Ref Line Y:"))
            self.ref_line_input = QLineEdit(self.chart_settings['ref_line'])
            self.ref_line_input.setPlaceholderText("e.g. 0")
            self.ref_line_input.setFixedWidth(70)
            options2_layout.addWidget(self.ref_line_input)

            options2_layout.addStretch()
            custom_layout.addLayout(options2_layout)
        ...
        def create_chart_from_dialog(self, dialog):
            ...
            x_column = self.x_axis_combo.currentData()
            ...
            ref_line_text = self.ref_line_input.text().strip()
            ...
            # Apply sort (by first selected Y column)
            if sort_order != "None" and y_columns:
                sort_col = y_columns[0]
                indices = sorted(range(len(x_data)),
                                 key=lambda i: y_data[sort_col][i],
                                 reverse=(sort_order == "Descending"))
                x_data = [x_data[i] for i in indices]
                x_data_full = [x_data_full[i] for i in indices]
                for col in y_columns:
                    y_data[col] = [y_data[col][i] for i in indices]
            ...
            for y_col in y_columns:
                ...
                if chart_type == "Line Chart":
                    ax.plot(x_data, values, marker='o', label=y_label, linewidth=2)
                    if show_data_labels:
                        for xi, yi in enumerate(values):
                            ax.annotate(f'{yi:.2f}', (xi, yi),
                                        textcoords="offset points", xytext=(0, 6),
                                        ha='center', fontsize=label_font - 3)
                elif chart_type == "Bar Chart":
                    x_pos = np.arange(len(x_data))
                    bar_width = 0.8 / len(y_columns)
                    offset = (y_columns.index(y_col) - len(y_columns)/2) * bar_width
                    bars = ax.bar(x_pos + offset, values, width=bar_width, label=y_label)
                    ax.set_xticks(x_pos)
                    ...
                    if show_data_labels:
                        for bar, val in zip(bars, values):
                            ...
                            ax.text(bar.get_x() + bar.get_width() / 2., h,
                                    ...
                                    fontsize=label_font - 3)
                elif chart_type == "Scatter Plot":
                    ax.scatter(x_data, values, label=y_label, s=100, alpha=0.6)
                    if show_data_labels:
                        for xi, yi in enumerate(values):
                            ax.annotate(f'{yi:.2f}', (xi, yi),
                                        textcoords="offset points", xytext=(5, 5),
                                        ha='left', fontsize=label_font - 3)
                elif chart_type == "Horizontal Bar Chart":
                    y_pos = np.arange(len(x_data))
                    bar_height = 0.8 / len(y_columns)
                    offset = (y_columns.index(y_col) - len(y_columns)/2) * bar_height
                    hbars = ax.barh(y_pos + offset, values, height=bar_height, label=y_label)
                    ax.set_yticks(y_pos)
                    ...
                    ax.fill_between(range(len(x_data)), values, alpha=0.5, label=y_label)
                    ax.plot(x_data, values, linewidth=2)
                    if show_data_labels:
                        for xi, yi in enumerate(values):
                            ax.annotate(f'{yi:.2f}', (xi, yi),
                                        textcoords="offset points", xytext=(0, 6),
            ...
            if ref_line_text:
                try:
                    ref_val = float(ref_line_text)
                    ax.axhline(y=ref_val, color='red', linestyle='--', linewidth=1.5,
                               label=f'Ref: {ref_val}', alpha=0.8)
                    ax.legend(fontsize=legend_font, loc='best')
                except ValueError:
                    pass
