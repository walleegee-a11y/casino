...
def load_yaml_config(config_path: Optional[str] = None) -> Optional[Dict]:
    """Load YAML configuration file and expand templates
    ...
    Returns:
        Parsed YAML configuration as dictionary with templates expanded, or None if loading fails
    """
    ...
    try:
        with open(config_path, 'r', encoding='utf-8') as f:
            ...
            # Expand YAML templates (task_mappings -> tasks)
            config = _expand_yaml_templates(config)
...
def _expand_yaml_templates(config: Dict) -> Dict:
    """Expand template-based task definitions (simplified version from config.py)

    Args:
        config: Raw YAML configuration

    Returns:
        Configuration with templates expanded into tasks
    """
    ...
    for task_key, mapping in mappings.items():
        ...
        if template_name in templates:
            ...
            # Simple template expansion - substitute {task_name}
            expanded_task = _substitute_task_name(template, task_name)
            tasks[task_key] = expanded_task
...
def _substitute_task_name(template: Any, task_name: str) -> Any:
    """Recursively substitute {task_name} in a template structure

    Args:
        template: Template structure (can be dict, list, string, etc.)
        task_name: Task name to substitute

    Returns:
        Structure with {task_name} replaced
    """
    if isinstance(template, dict):
        return {k: _substitute_task_name(v, task_name) for k, v in template.items()}
    elif isinstance(template, list):
        return [_substitute_task_name(item, task_name) for item in template]
...
def matches_template(keyword: str, template_name: str) -> bool:
    ...
    Automatically handles APR task prefixes (e.g., cts_inn_, place_inn_).
    ...
    Examples:
        ...
        True
        >>> matches_template("cts_inn_misn_ff_0p99v_m40c_Cbest_s_wns_all", "{mode}_{corner}_s_wns_{path_type}")
        True  # Strips cts_inn_ prefix before matching
    ...
    # Strip APR task prefix if present (for combined APR tasks)
    # Prefixes: place_inn_, cts_inn_, postcts_inn_, route_inn_, postroute_inn_, chipfinish_inn_, init_inn_
    apr_task_prefixes = ['place_inn_', 'cts_inn_', 'postcts_inn_', 'route_inn_', 'postroute_inn_', 'chipfinish_inn_', 'init_inn_']
    keyword_without_task = keyword
    for prefix in apr_task_prefixes:
        if keyword.startswith(prefix):
            keyword_without_task = keyword[len(prefix):]
            break
    ...
    return bool(regex.match(keyword_without_task))
