...
class ReportTableWidget(QWidget):
    ...
    def _extract_categories_from_keywords(self, keywords: Dict[str, Any]) -> List[str]:
        ...
        # Get full task data for accessing keyword metadata (like 'group' field)
        task_data_keywords = self.current_run_data.get('task_data', {}).get('keywords', {})

        # For each keyword, determine its group by:
        # 1. Check if keyword metadata has explicit 'group' field (for dynamically generated keywords)
        # 2. Check if it exists in YAML keyword_groups (exact match)
        # 3. If not found, use the keyword parser to infer the group
        for kw_name in keywords.keys():
            found_group = None

            # NEW: Check if keyword has explicit group field in metadata (for runtime-generated keywords)
            if kw_name in task_data_keywords and isinstance(task_data_keywords[kw_name], dict):
                explicit_group = task_data_keywords[kw_name].get('group', '')
                if explicit_group:
                    present_groups.add(explicit_group)
                    found_group = explicit_group
                    print(f"[DEBUG] Keyword '{kw_name}' -> group '{found_group}' (explicit metadata)")

            # Try exact match in YAML groups
            if not found_group:
            ...
            if not found_group:
                ...
                if "explicit metadata" not in str(found_group):  # Only print if not already logged
    ...
    def _infer_group_from_parsed_keyword(self, parsed: Any) -> Optional[str]:
        ...
        if isinstance(parsed, STAKeyword):
            # Check multiple ways to detect APR keywords:
            # 1. Has task field set (combined APR tasks like cts_inn_...)
            # 2. Keyword name starts with APR task prefix (backward compatibility)
            # 3. Mode is an APR mode (legacy)

            has_task = bool(parsed.task)  # Check if task field is populated

            apr_task_prefixes = ['place_', 'cts_', 'postcts_', 'route_', 'postroute_', 'chipfinish_', 'init_']
            is_apr_keyword = any(parsed.original_name.startswith(prefix) for prefix in apr_task_prefixes)

            # Also check legacy APR modes (for backward compatibility with old keywords)
            ...
            is_legacy_apr = parsed.mode in apr_modes

            if has_task or is_apr_keyword or is_legacy_apr:
                # APR keywords (e.g., place_func_ss_0p72v_m40c_Cmax_s_wns_all or legacy apr_inn_s_wns)
    ...
    def _populate_sta_timing_table(self, filter_timing=None):
        ...
        # Check both legacy APR modes and new APR keyword format (starting with task name)
        apr_modes = {'apr', 'init', 'place', 'cts', 'postcts', 'route', 'postroute', 'chipfinish'}
        apr_task_prefixes = ['place_', 'cts_', 'postcts_', 'route_', 'postroute_', 'chipfinish_', 'init_']

        filtered_sta_keywords = {}
        for mode, mode_data in grouping.sta_keywords.items():
            # Skip if mode is APR mode (legacy)
            if mode in apr_modes:
                continue

            # Skip if any keyword in this mode starts with APR task prefix (new format)
            has_apr_keywords = False
            for corner_data in mode_data.values():
                for timing_data in corner_data.values():
                    for metric_data in timing_data.values():
                        for kw in metric_data:
                            if any(kw.original_name.startswith(prefix) for prefix in apr_task_prefixes):
                                has_apr_keywords = True
                                break
                        if has_apr_keywords:
                            break
                    if has_apr_keywords:
    ...
    def _populate_apr_timing_table(self, filter_timing=None):
        ...
        apr_keywords = {}
        for mode, mode_data in grouping.sta_keywords.items():
            # Include if mode is APR mode (legacy)
            if mode in apr_modes:
                apr_keywords[mode] = mode_data
                continue

            # Include if any keyword in this mode starts with APR task prefix (new format)
            has_apr_keywords = False
            for corner_data in mode_data.values():
                for timing_data in corner_data.values():
                    for metric_data in timing_data.values():
        ...
        # Collect all unique path types and check if any keywords have task info
        path_types = set()
        has_task_info = False
            ...
            for corner_data in mode_data.values():
                for timing_data in corner_data.values():
                    for metric_keywords in timing_data.values():
                        for kw in metric_keywords:
                            path_types.add(kw.path_type)
                            if kw.task:  # Check if task field is populated
                                has_task_info = True

        path_types = sorted(path_types, key=lambda x: (x != 'all', x))  # 'all' first

        # Set up table columns: Task | Mode | Corner | Timing | Metric | {path_types} OR Mode | Corner | Timing | Metric | {path_types}
        if has_task_info:
            headers = ["Task", "Mode", "Corner", "Timing", "Metric"] + path_types
        else:
            headers = ["Mode", "Corner", "Timing", "Metric"] + path_types
            ...
            for corner in sorted(apr_keywords[mode].keys(), key=self.parser.natural_sort_key):
                for timing_type in timing_types:
                    ...
                    for metric in ['wns', 'tns', 'num']:
                        ...
                        # Get task from first keyword in this metric's list
                        kw_list = timing_data[metric]
                        task_name = kw_list[0].task if kw_list and kw_list[0].task else ""

                        # Build row data with Task column if has_task_info is True
                        if has_task_info:
                            row_data = {
                                'Task': task_name,
                                'Mode': mode,
