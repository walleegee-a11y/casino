...
class STAKeyword:
    ...
    task: str = ""  # APR task name for combined views (e.g., 'cts_inn', 'place_inn')
...
class KeywordParser:
    ...
    def _parse_sta_keyword(self, keyword_name: str) -> Optional[STAKeyword]:
        ...
        Patterns:
        1. Combined APR tasks: {task}_inn_{mode}_{corner}_{timing}_{metric}_{path_type}
           Example: cts_inn_misn_ss_0p72v_m40c_Cmax_s_wns_all
        2. APR format: apr_inn_{timing}_{metric}_{mode}_{corner...}
           Example: apr_inn_h_wns_misn_ff_0p99v_m40c_Cbest
        3. Standard format: {mode}_{corner}_{timing}_{metric}_{path_type}
           Example: misn_ff_0p99v_m40c_Cbest_s_wns_all
        """
        # Known path types from APR summary tables
        known_path_types = ['all', 'reg2reg', 'reg2cgate', 'in2reg', 'reg2out', 'in2out', 'default']

        # APR task prefixes for detecting combined task keywords
        apr_task_prefixes = ['place_inn', 'cts_inn', 'postcts_inn', 'route_inn', 'postroute_inn', 'chipfinish_inn', 'init_inn']

        # Check if keyword starts with APR task prefix (combined tasks view)
        task_prefix = ""
        keyword_without_task = keyword_name
        for prefix in apr_task_prefixes:
            if keyword_name.startswith(prefix + '_'):
                task_prefix = prefix
                keyword_without_task = keyword_name[len(prefix) + 1:]  # Remove "task_inn_" prefix
                break

        # Pattern 1: APR keywords from dynamic_table_row extraction
        # Format: apr_inn_{timing}_{metric}_{path_type}
        # or: {task}_inn_{timing}_{metric}_{path_type}
        # Examples:
        #   apr_inn_h_wns_all
        #   apr_inn_s_tns_reg2reg
        #   route_inn_h_wns_in2reg

        apr_match = re.match(r'^(\w+_inn)_(s|h)_(wns|tns|nov)_(.+)$', keyword_name)
        if apr_match:
            task_prefix = apr_match.group(1)  # e.g., 'apr_inn', 'route_inn'
            timing_type = apr_match.group(2)  # 's' or 'h'
            metric = apr_match.group(3)  # 'wns', 'tns', 'nov'
            suffix = apr_match.group(4)  # Could be path_type or mode_corner_path_type

            # Check if suffix is a known path_type (simple case - no mode/corner)
            if suffix in known_path_types:
                # Simple APR keyword without mode/corner
                # Use task_prefix as mode (e.g., "apr", "route", "cts")
                mode = task_prefix.replace('_inn', '') if task_prefix.endswith('_inn') else task_prefix
                corner = ''
                path_type = suffix
            else:
                # Complex case: suffix contains mode_corner_path_type
                # Try to extract path_type from the end
                parts = suffix.split('_')
                if parts[-1] in known_path_types:
                    # Last part is path_type
                    path_type = parts[-1]
                    mode_corner_part = '_'.join(parts[:-1])

                    # Split mode_corner_part into mode and corner
                    mode_parts = mode_corner_part.split('_')
                    if len(mode_parts) >= 2:
                        mode = mode_parts[0]
                        corner = '_'.join(mode_parts[1:])
                    else:
                        mode = mode_corner_part
                        corner = ''
                else:
                    # Fallback: treat entire suffix as mode/corner, use 'all' as path_type
                    mode_parts = suffix.split('_')
                    if len(mode_parts) >= 2:
                        mode = mode_parts[0]
                        corner = '_'.join(mode_parts[1:])
                    else:
            ...
            return STAKeyword(
                ...
                metric=metric,
                path_type=path_type,
                task=""  # No task for legacy APR keywords
            )

        # Pattern 2: Standard STA keywords (use keyword_without_task to skip task prefix if present)
        ...
        for pattern in sta_patterns:
            match = re.match(pattern, keyword_without_task)  # Use keyword WITHOUT task prefix
            if match:
                ...
                return STAKeyword(
                    ...
                    metric=metric,
                    path_type=path_type,
                    task=task_prefix  # Include task if it was detected
                )
