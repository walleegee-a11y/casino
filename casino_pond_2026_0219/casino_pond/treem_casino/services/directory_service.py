...
class DirectoryScanner(QThread):
    ...
    def _scan_with_progress(self, path: Path, max_depth: int, current_depth: int = 0) -> DirectoryHierarchy:
        """Scan directory hierarchy with progress reporting - OPTIMIZED with improved symlink handling."""
        if self._cancelled:
            return None
        ...
        # IMPROVED: Check if this is a symlink
        is_symlink = path.is_symlink()

        # IMPROVED: For symlinks, check if already visited - if so, still show it but don't recurse
        already_visited = real_path in self._visited_real_paths

        if already_visited and not is_symlink:
            # For non-symlinks, skip if already visited (shouldn't happen in normal case)
            return None

        # Mark as visited (before recursion to detect cycles)
        ...
        # IMPROVED: Load children if within depth limit and not already visited
        # For symlinks pointing to already-visited targets, show the symlink but don't recurse
        should_load_children = (
            current_depth < max_depth
            and not self._cancelled
            and not already_visited  # Don't recurse into already-visited paths
        )
            ...
            with os.scandir(str(path)) as entries:
                for entry in entries:
                    ...
                    try:
                        ...
                        logger.warning(f"Skipped entry '{entry.name}' in {path}: {type(e).__name__}: {e}")
            ...
            for child_path in child_paths:
                ...
                try:
                    ...
                    if child_hierarchy:
                        ...
                        # Log when _scan_with_progress returns None (circular reference or error)
                        logger.info(f"Skipped child hierarchy for '{child_path.name}' (likely circular reference or already visited)")

                except (OSError, PermissionError) as e:
                    logger.warning(f"Failed to scan child '{child_path.name}': {type(e).__name__}: {e}")
                    continue  # Skip inaccessible directories

        except (OSError, PermissionError) as e:
            logger.error(f"Cannot read directory {path}: {type(e).__name__}: {e}")
