...
class EnhancedTreeView(QTreeView):
    ...
    def setup_view(self):
        ...
        self.setHeaderHidden(False)  # Show column headers (Name, Modified, Owner)
        self.setSizePolicy(QSizePolicy.Expanding, QSizePolicy.Expanding)
        self.setMinimumHeight(self.config.ui.tree_minimum_height)
        self.setStyleSheet(f"""
            QTreeView {{
                background-color: {self.config.colors.tree_background};
            }}
            QHeaderView::section {{
        ...
        # Column sizing will be applied after model is set
        self.setSortingEnabled(False)  # Sorting handled by our model, not Qt
    ...
    def selectionChanged(self, selected, deselected):
        ...
        # Emit custom signal with selected paths (column 0 only to avoid duplicates)
        selected_paths = []
