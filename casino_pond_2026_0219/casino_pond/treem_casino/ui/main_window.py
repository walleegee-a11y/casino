...
class MainWindow(QMainWindow):
    ...
    def __init__(self, config: AppConfig):
        ...
        self._fs_debounce_timer.setInterval(1000)  # 1s debounce
        self._fs_debounce_timer.timeout.connect(self._on_fs_debounce_timeout)
...
#       self.history_service.add_terminal_operation(path)
    ...
    def _setup_fs_watcher(self):
        ...
        if self.fs_watcher:
            ...
            self.fs_watcher.deleteLater()

        self.fs_watcher = QFileSystemWatcher(self)
        self.fs_watcher.addPath(base_path)

        # Also watch immediate subdirectories (depth 1) for create/delete detection
        try:
            import os
            with os.scandir(base_path) as entries:
                for entry in entries:
                    if entry.is_dir(follow_symlinks=True):
                        self.fs_watcher.addPath(entry.path)
        except (OSError, PermissionError):
            pass

        self.fs_watcher.directoryChanged.connect(self._on_fs_directory_changed)
        logger.info(f"Filesystem watcher set on: {base_path}")
    ...
    def _on_fs_debounce_timeout(self):
        """Debounced auto-refresh after filesystem changes."""
        logger.info("Auto-refreshing after filesystem change")
        self.show_status_message("Auto-refreshing: filesystem change detected...", 3000)
        self.refresh_directory_tree()
    ...
    def save_expand_state(self) -> set:
        ...
        if not self.tree_model:
            return expanded
        self._collect_expanded_paths(QModelIndex(), expanded)
        return expanded

    def _collect_expanded_paths(self, parent_index, expanded: set):
        """Recursively collect expanded paths."""
        if not self.tree_model:
            return
        row_count = self.tree_model.rowCount(parent_index)
        for row in range(row_count):
            child_index = self.tree_model.index(row, 0, parent_index)
            if child_index.isValid() and self.tree_view.isExpanded(child_index):
                path_data = child_index.data(Qt.UserRole)
                if path_data:
                    expanded.add(Path(path_data))
                self._collect_expanded_paths(child_index, expanded)

    def restore_expand_state(self, expanded_paths: set):
        """Restore expand state after model refresh."""
        if not self.tree_model or not expanded_paths:
            return
        for path in expanded_paths:
            index = self.tree_model.find_path_index(path)
            if index and index.isValid():
    ...
    def update_tree_view(self):
        ...
        # PERF: Save expand state before rebuilding
        saved_expanded = self.save_expand_state()
        saved_selection = None
        if self.tree_view.currentIndex().isValid():
            saved_selection = self.tree_view.currentIndex().data(Qt.UserRole)
        ...
        # Set column widths for multi-column view (all adjustable by user)
        ...
        # PERF: Restore expand state after rebuilding
        self.restore_expand_state(saved_expanded)
        if saved_selection:
            self.select_directory_path(saved_selection)
    ...
    def auto_expand_user_workspace(self):
        ...
        index = self.tree_model.find_pattern_index(user_pattern)

        if index and index.isValid():
            self.tree_view.setExpanded(index, True)
            self.tree_view.setCurrentIndex(index)

            item_depth = self.tree_model.get_item_depth(index)
    ...
    def on_user_filter_toggled(self, me_only: bool):
        """Handle user filter toggle — resets depth to 0."""
        self.current_filter.show_only_user_workspace = me_only
        if me_only:
            self.current_filter.user_name = getpass.getuser()

        self.update_tree_view()
        # Reset to depth 0 (collapsed) on filter toggle
        self.set_depth_level(0)
        self.depth_controls.set_current_depth(0)
    ...
    def on_selection_changed(self, selected_paths: List[Path]):
        ...
        if selected_paths and self.tree_model:
            ...
            source_index = self.tree_model.find_path_index(first_path)
            if source_index and source_index.isValid():
                depth = self.tree_model.get_item_depth(source_index)
    ...
    def get_selected_paths(self) -> List[Path]:
        """Get currently selected directory paths (column 0 only to avoid duplicates)."""
        ...
        for index in self.tree_view.selectedIndexes():
            if index.column() != 0:
                continue  # Skip non-name columns to avoid 3x duplicates
