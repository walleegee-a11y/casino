        ...
        try:
            if self.path.exists():
                ...
                try:
                    ...
                    self.owner = str(stat_info.st_uid)
...
class DirectoryHierarchy:
    ...
    def from_path(
        ...
        """Create hierarchy from a filesystem path with improved symlink handling."""
        ...
        already_visited = real_path in visited_real_paths

        if already_visited and not is_symlink:
            # For non-symlinks, skip if already visited
            return None
        ...
        # IMPROVED: Load children only if within depth limit and not already visited
        # For symlinks pointing to already-visited targets, show the symlink but don't recurse
        should_load_children = current_depth < max_depth and not already_visited
