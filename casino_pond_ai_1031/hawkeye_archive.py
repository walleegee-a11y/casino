...
class HawkeyeArchive:
    ...
    def _archive_run_data(self, run_info: Dict[str, str], run_data: Dict[str, Any]) -> bool:
        ...
        try:
            ...
            # Count keywords - handle both old (direct tasks) and new (jobs->tasks) structure
            keyword_count = 0
            tasks_dict = {}
            ...
            if self.use_sqlite:
                with sqlite3.connect(self.db_path) as conn:
                    ...
                    # Insert task results - handle both old and new structure
                    tasks_to_process = []
                    if 'jobs' in run_data:
                        # New structure: jobs->job_name->tasks
                        for job_name, job_data in run_data['jobs'].items():
                            if 'tasks' in job_data:
                                for task_name, task_data in job_data['tasks'].items():
                                    tasks_to_process.append((task_name, task_data, job_name))
                    elif 'tasks' in run_data:
                        # Old structure: direct tasks
                        for task_name, task_data in run_data['tasks'].items():
                            tasks_to_process.append((task_name, task_data, task_data.get('job', 'unknown')))

                    for task_name, task_data, job_name in tasks_to_process:
                        cursor.execute('''
        ...
        try:
            ...
            if len(data_files) > 0:
                if self.use_sqlite:
                    ...
                    # For JSON mode, check against metadata
                    try:
                        with open(self.metadata_json_path, 'r') as f:
                            metadata = json.load(f)
                        expected_files = {entry.get('data_file') for entry in metadata.get('archive_entries', [])}
                        orphaned_files = [f for f in data_files if f not in expected_files]
                    except:
                        orphaned_files = data_files

            # If orphaned files exist, repair them
            if len(orphaned_files) > 0:
                print(f"- Auto-repair: Found {len(orphaned_files)} orphaned data files (out of {len(data_files)} total), restoring metadata...")

                repaired_count = 0
                for data_file in orphaned_files:
                    try:
                        ...
                        # Check if path info is stored in the tasks data - handle both structures
                        if not full_path:
                            tasks_to_check = {}
                            if 'jobs' in run_data:
                                # New structure: jobs->job_name->tasks
                                for job_name, job_data in run_data['jobs'].items():
                                    if 'tasks' in job_data:
                                        tasks_to_check.update(job_data['tasks'])
                            elif 'tasks' in run_data:
                                # Old structure: direct tasks
                                tasks_to_check = run_data['tasks']

                            for task_name, task_data in tasks_to_check.items():
                ...
                if repaired_count > 0:
                    print(f"- Auto-repair completed: {repaired_count}/{len(orphaned_files)} entries restored")
                else:
                    print(f"- Auto-repair failed: No orphaned entries could be restored")
            else:
                print(f"- Archive metadata OK: {entry_count} entries found, {len(data_files)} data files, no orphans detected")
