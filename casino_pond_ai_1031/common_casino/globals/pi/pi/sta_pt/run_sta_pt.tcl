            ...
            while { [gets $fin line] >= 0 } {
                ...
                foreach sub_block $sub_block_list {
                    if { $sub_block == $sub_module } {
                        ...
                        set sub_x_coordinate [expr {${sub_x_coordinate} * 1000}]
                        set sub_y_coordinate [expr {${sub_y_coordinate} * 1000}]

                        if { [regexp {^M?(X|Y)?R?(0|90|180|270)?$} $sub_orientation match flip rotate] } {
                            if { $flip eq "" } {
                                set sub_flip none
                            } else {
