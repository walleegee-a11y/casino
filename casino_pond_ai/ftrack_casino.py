...
# Simplified pastel button colors
BUTTON_STYLES = {
    "default": {
    ...
    "action": {
        "background": "#E0FFE0",  # Mint
        "border": "#B0E0B0",
        "hover": "#D0FFD0",
        "pressed": "#C0F0C0"
    },
    "danger": {
        "background": "#FFE6E6",  # Light Rose
        "border": "#FFB0B0",
        "hover": "#FFD8D8",
        "pressed": "#FFC0C0"
    },
...
def style_button(button, style_type="default"):
    """Apply consistent pastel button styling"""
...
class FastTrackGUI(QWidget):
    ...
    STATUS_COLORS = {
        "Open": QColor("#f8d7da"),      # Light red
        "In Progress": QColor("#fff3cd"), # Light yellow
        "Resolved": QColor("#d4edda"),   # Light green
        "Closed": QColor("#d1ecf1")      # Light blue
    }
    SEVERITY_COLORS = {
        "Critical": QColor("#dc3545"),  # Red
        "Major": QColor("#fd7e14"),     # Orange
        "Minor": QColor("#ffc107"),     # Yellow
        "Enhancement": QColor("#28a745"), # Green
        "Info": QColor("#e9ecef")       # Light Gray
    }
    ...
    def _init_ui(self):
        layout = QVBoxLayout()
        layout.addWidget(QLabel(f"Project Dir: {self.proj_dir}"))
        layout.addWidget(QLabel(f"Assigner: {self.assigner}"))
        ...
        # Attachments
        right_content.addWidget(QLabel("Attachments:"))
        self.attach_list = QListWidget(); right_content.addWidget(self.attach_list)
    ...
    def _view_issues(self):
        ...
        def on_cell_clicked(row, col):
            ...
            data = yaml.safe_load(open(path))
            dlg2 = QDialog(dlg)
            dlg2.setWindowTitle("Description")
            v = QVBoxLayout(dlg2)
            te = QTextEdit()
            te.setReadOnly(True)
            ...
            btn.clicked.connect(dlg2.close)
            v.addWidget(btn)
            dlg2.resize(600,400)
            dlg2.exec_()
...
<head>
    ...
    <style>
        ...
        .severity-info {{ background-color: #e9ecef; }}
